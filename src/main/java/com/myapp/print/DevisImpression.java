...
public class DevisImpression implements Printable {
    ...
    private String remiseGlobaleStr;
    ...
    public DevisImpression(String numero, String date, String nom, String prenom,
                           String adresse, String tel, String matricule,
                           DefaultTableModel model, String remiseStr,
                           ImageIcon logo, double ht, double ttc, double mntRemise) {
        this.numeroDevis      = numero;
        this.dateDevis        = date;
        this.nomClient        = nom;
        ...
        this.model            = model;
        this.remiseGlobaleStr = remiseStr;
        this.logoIcon         = logo;
        this.totalHT          = ht;
        this.totalTTC         = ttc;
        this.montantRemise    = mntRemise;
    ...
    private int printNumeroDate(Graphics2D g2d, int margin, int y, int contentWidth) {
        ...
        g2d.drawString("DEVIS N°: " + numeroDevis, margin + 10, y + 21);

        g2d.setFont(new Font("Arial", Font.PLAIN, 12));
        String dateText = "Date: " + dateDevis;
    ...
    private int printClientSection(Graphics2D g2d, int margin, int y, int contentWidth) {
        ...
        g2d.drawLine(margin + padding, y + 20, margin + 180, y + 20);
    ...
    private int printMontantEnLettres(Graphics2D g2d, int margin, int y, int contentWidth) {
        ...
        g2d.drawString("Arrete le present devis a la somme de :", margin, y);
        ...
        if (lettres.length() > 60) {
            ...
            if (parts.length > 1) { g2d.drawString(parts[1], margin, y + 30); return y + 45; }
        } else {
    ...
    private String convertirMontantEnLettres(double montant) {
        try {
            ...
            else                         resultat.append("zero dinar");

            if (partieDecimale > 0) {
                resultat.append(" et ");
                if      (partieDecimale == 1)   resultat.append("un millime");
                else if (partieDecimale < 1000) resultat.append(convertirNombreEnLettres(partieDecimale)).append(" millimes");
                else {
    ...
    private String convertirNombreEnLettres(int nombre) {
        ...
        if (nombre < 100) {
            int dizaine = nombre / 10, unite = nombre % 10;
        ...
        if (nombre < 1000) {
            int centaines = nombre / 100, reste = nombre % 100;
        ...
        if (nombre < 1000000) {
            int milliers = nombre / 1000, reste = nombre % 1000;
        ...
        if (nombre < 1000000000) {
            int millions = nombre / 1000000, reste = nombre % 1000000;
    ...
    public void imprimer() throws PrinterException {
        ...
        job.setJobName("Devis " + numeroDevis);
