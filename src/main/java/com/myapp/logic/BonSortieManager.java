...
public class BonSortieManager {
    ...
    public BonSortieManager(DefaultTableModel model) {
        this.model = model;
        this.df = new DecimalFormat("#,##0.00", new DecimalFormatSymbols(Locale.FRANCE));
        System.out.println("✅ BonSortieManager initialisé");
    }
    ...
    public void ajouterArticle(String designation, int quantiteAjout, double prixTTC, double remisePourcentage) {
        try {
            ...
            System.err.println("❌ Erreur dans BonSortieManager: " + e.getMessage());
     ...
     * 
     * MONTANT REMISE = somme des remises par ligne en DT
     * Remise ligne en DT = TTC brut (PU TTC de base × Qté) - TTC affiché (col 8)
     * 
    ...
    public CalculTotaux calculerTotaux() {
        ...
        return new CalculTotaux(totalHT, totalTTC, totalRemiseLignes);
    }

    public boolean validerBonSortie(String numero, String date, String client) {
        if (this.model.getRowCount() == 0) {
            JOptionPane.showMessageDialog(null, "Le Bon de Sortie est vide.", "Attention", JOptionPane.WARNING_MESSAGE);
            return false;
        }
        ...
        System.out.println("✅ Bon de Sortie validé - Stock vérifié");
        return true;
    }
    ...
    public void viderTableau() {
        ...
        System.out.println("🗑️ Tableau vidé");
    }
